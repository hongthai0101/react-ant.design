...
const DetailDrawer: React.FC<Props> = function({
  ...
  const record: Record<string, any> = useMemo(() => {
    ...
    return {
      ...data,
      title: `${data.name} - ${detail.company.name}`
    }
  }, [detail])
  ...
  return (
    <Drawer
      ...
      destroyOnClose
    >
      <p className="pl20 mb20">Date: {detail.__createdAt__}</p>
      <Collapse
        defaultActiveKey={defaultActiveKey}
        expandIconPosition="right"
      >
        <Panel header={record.doneTitle} key="1">
          <pre>{detail.doneContent || 'Without'}</pre>
        </Panel>
        <Panel header={record.undoneTitle} key="2">
          <pre>{detail.undoneContent || 'Without'}</pre>
        </Panel>
        <Panel header={record.planTitle} key="3">
          <pre>{detail.planContent || 'Without'}</pre>
        </Panel>
        <Panel header={record.summaryTitle} key="5">
          <pre>{detail.summaryContent || 'Without'}</pre>
        </Panel>
      </Collapse>
    </Drawer>
  )
